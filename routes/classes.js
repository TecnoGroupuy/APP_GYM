...
router.get("/", async (_req, res) => {
  try {
    const classes = await ClassModel.find({ active: true }).sort({ day: 1, time: 1 });
    return res.json({ classes });
  } catch (error) {
    return res.status(500).json({ message: "Error al listar clases", error: error.message });
  }
});
...
router.post(
  ...
  [
    ...
    body("day").notEmpty().withMessage("Dia requerido"),
    body("time").notEmpty().withMessage("Hora requerida"),
    body("trainer").notEmpty().withMessage("Entrenador requerido"),
    body("spots").isInt({ min: 1 }).withMessage("Cupos invalidos")
  ],
  async (req, res) => {
    ...
    try {
      ...
      return res.status(500).json({ message: "Error al crear clase", error: error.message });
    }
  }
);
